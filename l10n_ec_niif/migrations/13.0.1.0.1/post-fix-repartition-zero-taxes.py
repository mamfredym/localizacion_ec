...
def migrate(cr, version):
    ...
    zero_taxes = tax_model.search(
        [("amount", "=", 0), ("tax_group_id", "in", groups.ids)]
    )
    for tax in zero_taxes:
        if not tax.invoice_repartition_line_ids.filtered(
        ...
        if not tax.refund_repartition_line_ids.filtered(
            lambda x: x.repartition_type == "tax"
        ):
            repartition_line.create(
                {
