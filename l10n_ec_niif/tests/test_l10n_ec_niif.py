...
class TestModule(common.TransactionCase):
    ...
    def test_creation_data(self):
        self.assertEqual(self.test_agency1.name, "Agency001",
                         'Agency name does not match')
        self.assertEqual(self.test_agency1.number, "999",
                         'Agency number does not match')
        # SUCCESSFUL AGENCY

        self.assertEqual(self.test_pofe1.name, "PofE001",
                         'PofE name does not match')
        self.assertEqual(self.test_pofe1.number, "001",
                         'PofE number does not match')
        self.assertEqual(self.test_pofe1.agency_id.id,
                         self.test_agency1.id, 'Agency/PofE relationship is wrong')
        self.assertIn(self.test_pofe1.type_emission, [
                      'electronic', 'pre_printed', 'auto_printer'], 'The type of issue is not within the established rates')
        # 2 SUCCESSFULLY POINT OF EMISSION

        self.assertEqual(self.test_auth1.number, "AUTH001",
                         'Authorization name does not match')
        self.assertEqual(self.test_auth1.start_date, datetime.date(
            2020, 8, 1), 'Start date does not match')
        self.assertEqual(self.test_auth1.expiration_date, datetime.date(
            2020, 8, 20), 'The effective date does not match')
        # 3 SUCCESSFULLY AUTHORIZATION

        self.assertEqual(self.test_doc1.point_of_emission_id.id,
                         self.test_pofe1.id, 'The Document/PofE relationship is incorrect')
        self.assertEqual(self.test_doc1.authorization_id.id, self.test_auth1.id,
                         'The Document/authorization relationship is incorrect')
        self.assertEqual(self.test_doc1.first_sequence, 1,
                         'The first sequence number does not match')
        self.assertEqual(self.test_doc1.last_sequence, 100,
                         'The last sequence number does not match')
        self.assertIn(self.test_doc1.document_type, ['invoice', 'withholding', 'liquidation',
                                                     'credit_note', 'debit_note'], 'he document type is not within the established types')
        # 4 SUCCESSFULLY DOCUMENT

        self.assertEqual(self.test_invoice1.l10n_ec_point_of_emission_id.id,
                         self.test_pofe1.id, 'The PofE/Invoice relationship is incorrect')
        self.assertEqual(self.test_invoice1.l10n_ec_agency_id.id,
                         self.test_agency1.id, 'The Agency/Invoice relationship is incorrect')
        self.assertEqual(self.test_invoice1.l10n_ec_authorization_id.id,
                         self.test_auth1.id, 'The Authorization/Invoice relationship is incorrect')
        # 5 SUCCESSFUL INVOICE

    def test_relationship_active_fiel_agency_pofe(self):
        self.assertIs(self.test_pofe2.active, self.test_agency2.active,
