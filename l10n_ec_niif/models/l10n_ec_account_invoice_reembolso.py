...
class AccountInvoiceReembolso(models.Model):
    ...
    @api.depends('total_base_iva', 'total_base_iva0', 'total_base_no_iva', 'total_iva', 'total_ice')
    def _compute_total_invoice(self):
        for reembolso in self:
            reembolso.total_invoice = reembolso.total_base_iva + reembolso.total_base_iva0 + \
                                      reembolso.total_base_no_iva + reembolso.total_iva + reembolso.total_ice

    invoice_id = fields.Many2one('account.move', 'Liquidación de Compras', ondelete="cascade", index=True, auto_join=True)
    ...
    ], string='Tipo Documento', required=True, readonly=False, default='normal')
    ...
    def _check_number_invoice(self):
        ...
        padding_auth = "1,9"
        for reembolso in self:
            if reembolso.l10n_ec_partner_authorization_id and reembolso.l10n_ec_partner_authorization_id.padding > 0:
                padding_auth = reembolso.l10n_ec_partner_authorization_id.padding
            cadena='(\d{3})+\-(\d{3})+\-(\d{%s})' % (padding_auth)
            if not reembolso.l10n_ec_foreign and reembolso.document_number and not re.match(cadena, reembolso.document_number):
                raise ValidationError(_("The número de documento no es correcto, debe ser de la forma 00X-00X-000XXXXXX, X es un número"))
            if reembolso.document_type == 'normal':
                if not auth_s_model.check_number_document('invoice_reembolso', reembolso.document_number, reembolso.l10n_ec_partner_authorization_id, reembolso.date_invoice, reembolso.id, reembolso.l10n_ec_foreign):
                    raise ValidationError(_("Ya existe otro documento con el mismo número"))
            else:
                auth_s_model.validate_unique_document_partner('invoice_reembolso', reembolso.document_number,
                                                              reembolso.partner_id.id, util_model.ensure_id(reembolso))
    ...
    def _check_electronic_authorization(self):
        cadena='(\d{37}$)|(\d{49}$)'
        for reembolso in self:
            if reembolso.document_type == 'electronic' and reembolso.electronic_authorization:
                if len(reembolso.electronic_authorization) not in (37, 49):
                    raise ValidationError(_("El número de autorización electrónica es incorrecto, "
                                            "este debe ser de 37 or 49 digitos. Revise el reembolso"))
                if not re.match(cadena, reembolso.electronic_authorization):
                    raise ValidationError(_("La autorización electronica debe tener solo números, "
    ...
    def onchange_data_in(self):
                ...
                if len(number_data) == 3:
                    try:
                        ...
                        warning = {'title': 'Advertencia!!!',
                                   'message': _("The número de documento no es correcto, debe ser de la forma 00X-00X-000XXXXXX, X es un número")
                                   }
                        number_last = False
                    if number_last:
                        #cuando deberia ser el padding(9 por defecto)
                        invoice_number = "%s-%s-%s" % (number_data[0], number_data[1], auth_supplier_model.fill_padding(number_last, 9))
        ...
        if invoice_number and not self.partner_id:
            ...
            warning = {'title': _('Advertencia!!!'),
                       'message': _('Usted debe seleccionar primero la empresa para proceder con esta acción'),
                       }
            return {'domain':domain, 'warning': warning}
        if self.partner_id and self.partner_id.l10n_ec_foreign:
            return {'domain': domain, 'warning': warning}
        auth_data = auth_supplier_model.get_supplier_authorizations('invoice', self.partner_id.id, invoice_number, date_invoice)
        ...
        if auth_data.get('multi_auth', False):
            if self.l10n_ec_partner_authorization_id and self.l10n_ec_partner_authorization_id in auth_data.get('auth_ids', []):
                ...
                if number_to_check and int(number_to_check) >= auth_use.first_sequence and int(number_to_check) <= auth_use.last_sequence:
                    invoice_number = auth_use.agency + '-' + auth_use.printer_point + '-' + auth_supplier_model.fill_padding(invoice_number, auth_use.padding)
                    self.document_number = invoice_number
                    #si hay ids pasar el id para validar sin considerar el documento actual
                    auth_supplier_model.check_number_document('invoice_reembolso', invoice_number, auth_use,
                                                              date_invoice, util_model.ensure_id(self))
                else:
                    self.document_number = ''
            else:
                self.document_number = ''
        ...
        if len(invoice_number.split('-')) == 3 and auth_ids:
            ...
            #si hay ids pasar el id para validar sin considerar el documento actual
            auth_supplier_model.check_number_document('invoice_reembolso', invoice_number, auth,
                                                      date_invoice, util_model.ensure_id(self))
        return {'domain': domain, 'warning': warning}
