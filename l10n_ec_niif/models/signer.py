...
from OpenSSL.crypto import dump_certificate, load_pkcs12, sign
...
def main():
    ...
    exponent = (
        codecs.encode(codecs.decode(e, "hex_codec"), "base64")
        .decode()
        .replace("\n", "")
    )
    ...
    certificateX509_der_hash = (
        base64.b64encode(x509_cert.digest("sha1")).decode().replace("\n", "")
    )
    X509SerialNumber = str(cert.serial_number)
    issuer = x509_cert.get_issuer()
    issuer_name = "CN={},L={},OU={},O={},C={}".format(
