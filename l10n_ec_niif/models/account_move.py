...
class AccountMove(models.Model):
    ...
    l10n_ec_reembolso_ids = fields.One2many(
        comodel_name="l10n_ec.account.invoice.reembolso",
    ...
    def action_post(self):
        for move in self:
            if move.company_id.country_id.code == "EC":
                if (
                    move.l10n_ec_consumidor_final
                    and move.type == "out_invoice"
                    and float_compare(
                        move.amount_total,
                        move.company_id.l10n_ec_consumidor_final_limit,
                        precision_digits=2,
                    )
                    == 1
                ):
                ...
                if move.l10n_ec_consumidor_final and move.type == "out_refund":
                    raise UserError(
                        _(
                            "You can't make refund to final customer on ecuadorian company"
                        )
                    )
                if move.l10n_ec_consumidor_final and move.type in (
                    "in_refund",
                    "in_invoice",
                ):
                    raise UserError(
                ...
                if move.type == "in_invoice":
                    ...
                    for line in move.invoice_line_ids:
                        ...
                        iva_0_taxes = line.tax_ids.filtered(
                            lambda x: x.tax_group_id.id == iva_group.id
                            and x.amount == 0
                        )
                        ...
                        if move.partner_id.country_id.code == "EC":
                            if len(rent_withhold_taxes) == 0:
                                errors[line].append(
                                    _(
                                        "You must apply at least one income withholding tax"
                                    )
                                )
                        ...
                        if len(iva_taxes) > 1:
                            errors[line].append(
                                ...
                                % (
                                    " / ".join(
                                        t.description or t.name for t in iva_taxes
                                    )
                                )
                            )
                        if len(iva_0_taxes) > 1:
                            errors[line].append(
                                ...
                                % (
                                    " / ".join(
                                        t.description or t.name for t in iva_0_taxes
                                    )
                                )
                            )
                        if len(withhold_iva_taxes) > 1:
                            errors[line].append(
                                _(
                                    "You cannot have more than one VAT Withholding tax %s"
                                )
                                % (
                                    " / ".join(
                                        t.description or t.name
                                        for t in withhold_iva_taxes
                                    )
                                )
                            )
                        if len(rent_withhold_taxes) > 1:
                            errors[line].append(
                                _(
                                    "You cannot have more than one Rent Withholding tax %s"
                                )
                                % (
                                    " / ".join(
                                        t.description or t.name
                                        for t in rent_withhold_taxes
                                    )
                                )
                            )
                    ...
                    for line in move.invoice_line_ids:
                        for tax in line.tax_ids:
                            if tax.tax_group_id.id in (
                                ...
                                tax_type = (
                                    tax.tax_group_id.id == withhold_iva_group.id
                                    and "iva"
                                    or tax.tax_group_id.id == withhold_rent_group.id
                                    and "rent"
                                )
                                percent = (
                                    tax.tax_group_id.id == withhold_iva_group.id
                                ...
                                tax_data.setdefault(
                                    tax.id,
                                    {
                                        ...
                                        and base_tag_id.ids[0]
                                        or False,
                                        "tax_tag_id": tax_tag_id
                                        and tax_tag_id.ids[0]
                                        or False,
                                        "type": tax_type,
                                        "base_amount": 0.0,
                                        "tax_amount": 0.0,
                    ...
                    for tax_id in tax_data.keys():
                        ...
                        if current_tax.tax_group_id.id == withhold_iva_group.id:
                            tax_data[tax_id]["base_amount"] = tax_data[tax_id][
                                "tax_amount"
                            ] / (
                                percent_model.browse(
                                    tax_data[tax_id]["percent_id"]
                                ).percent
                                / 100.0
                            )
                            tax_data[tax_id][
    ...
    l10n_ec_withhold_count = fields.Integer(
        string="Withhold Count", compute="_get_l10n_ec_withhold_ids", store=False
    )
    ...
    def l10n_ec_get_info_liquidation(self, node):
        ...
        if (
            ...
            SubElement(
                ...
                sum([r.total_invoice for r in self.l10n_ec_reembolso_ids]),
                decimales=currency.decimal_places,
            )
            SubElement(
                infoLiquidacionCompra, "totalBaseImponibleReembolso"
            ).text = util_model.formato_numero(
                sum([r.total_base_iva for r in self.l10n_ec_reembolso_ids]),
                decimales=currency.decimal_places,
            )
            SubElement(
                infoLiquidacionCompra, "totalImpuestoReembolso"
            ).text = util_model.formato_numero(
                sum([r.l10n_ec_iva for r in self.l10n_ec_reembolso_ids])
                + sum([r.total_ice for r in self.l10n_ec_reembolso_ids]),
                decimales=currency.decimal_places,
            )
        # Definicion de Impuestos
        # xq no itero sobre los impuestos???'
        ...
        if self.l10n_ec_reembolso_ids:
            ...
            for reembolso in self.l10n_ec_reembolso_ids:
                ...
                if reembolso.partner_id.commercial_partner_id.l10n_ec_type_sri == "Ruc":
                    ...
                    reembolso.partner_id.commercial_partner_id.l10n_ec_type_sri
                    == "Cedula"
                ):
                    tipoIdentificacionComprador = "05"
                elif (
                ...
                fecha_emision = reembolso.date_invoice.strftime(
                ...
                SubElement(reembolso_detail, "numeroautorizacionDocReemb").text = (
                    ...
                    and reembolso.l10n_ec_partner_authorization_id.number
                    or reembolso.electronic_authorization
                )
                detalleImpuestos = SubElement(reembolso_detail, "detalleImpuestos")
                tarifa_iva = (
                    reembolso.total_base_iva
                    and round((reembolso.total_iva / reembolso.total_base_iva), 2)
                    or 0.0
                )
                tipo_iva = "2"
                if tarifa_iva == 0.14:
