...
class L10nECSriAuthorizationSupplier(models.Model):
        ...
        for auth in self:
            if auth.agency and not re.match(cadena, auth.agency):
                raise ValidationError(
                    _("Invalid Number Format, this must be like 001"))
    ...
    def _check_dates(self):
        for auth in self:
            if auth.start_date >= auth.expiration_date:
                raise ValidationError(_("The dates of authorization of supplier %s "
                                        "must be valid start: %s end: %s") % (auth.number,
                                                                              auth.start_date, auth.expiration_date))
        ...
        for auth in self:
            if auth.first_sequence < 0 and not auth.autoprinter:
                raise ValidationError(_(
                    "First sequence must be bigger than zero, please check authorization %s") % auth.display_name)
            ...
            if auth.first_sequence >= auth.last_sequence and not auth.autoprinter:
                raise ValidationError(_("First sequence must be bigger than last sequence, "
                                        "please check the authorization %s start: % last:%s") % (auth.display_name,
                                                                                                 auth.first_sequence,
                                                                                                 auth.last_sequence))
            elif not auth.autoprinter:
                args = [('partner_id', '=', auth.partner_id.id),
                        ('number', '=', auth.number),
                ...
                for other_auth in other_auth_recs:
                    if auth.first_sequence <= other_auth.first_sequence and auth.last_sequence >= other_auth.last_sequence:
                        is_valid = False
                    if auth.first_sequence >= other_auth.first_sequence and auth.last_sequence <= other_auth.last_sequence:
                        is_valid = False
                    ...
                    if other_auth.last_sequence >= auth.first_sequence >= other_auth.first_sequence:
                        is_valid = False
                if not is_valid:
                    raise ValidationError(_("There's another authorization in same periods, "
                                            "please check the authorization on partner %s") % auth.display_name)
    ...
    def _check_padding(self):
        for auth in self:
            if auth.padding < 0 or auth.padding > 9:
                raise ValidationError(
                    _("Padding number must be between 0 and 9"))
    ...
    def _check_document_in_use(self, vals):
        ...
        for auth in self:
            ...
            if has_change:
                ...
                if retention_model:
                    retention_recs = retention_model.search([  # TODO: filtrar estado???
                        ('state', 'not in', ('draft', 'canceled')),
                        ('authorization_sale_id', 'in', self.ids),
                    ])
                invoice_recs = invoice_model.search([  # TODO: filtrar estado???
    ...
    number = fields.Char('Number', size=10, required=True,
                         readonly=False, index=True, help="", )
    ...
    def check_number_document(self, invoice_type, number, authorization, date=None, res_id=None, foreign=False):
        if not invoice_type:
            raise Warning(
                _('You must declare document type to check authorization'))
        if not number:
            raise Warning(
        ...
        if number:
            if partner_id:
                ...
                other_recs = res_model.search(
                    domain + [(field_name, '=', number), ('partner_id', '=', partner_id)])
        if not foreign:
    ...
    def validate_unique_document_partner(self, invoice_type, number, partner_id, res_id=None):
        if not number or not partner_id:
            raise Warning(
                _("Check parameters of partner and number to continue"))
        if not invoice_type:
        ...
        try:
            ...
            num_doc = int(num_doc)
            number_criteria = [(field_name, 'like', '%s-%s-%%s' %
                                (num_shop, num_printer, num_doc))]
        except Exception as e:
            number_criteria = [(field_name, '=', str(number))]
        args = modules_mapping.get_domain(invoice_type, include_state=False) + [
            ('partner_id', '=', partner_id)] + number_criteria
        if res_id:
            args.append(('id', '!=', res_id))
        model_recs = number and res_model.search(args) or False
        if model_recs:
            raise Warning(_("There's another document type %s with number '%s' for partner %s") % (
    ...
    @api.model
    def get_supplier_authorizations(self, document_type, partner_id, number=None, date=None):
        partner_model = self.env['res.partner']
        res = {'message': '',
               'multi_auth': False,
               'res_number': '',
        ...
        if not auth_recs:
            ...
            if not auth_recs:
                if not seq_number:
                    message += _("There's no exist authorization for document type %s for partner %s with date %s, "
                                 "check if you should create authorization") % (model_description, partner.name, date)
                else:
                    message += _("There's no exist authorization for document type %s "
        ...
        if len(auth_recs) > 1:
            message += _("There is more than one match of active authorizations for the partner %s of type %s "
            ...
            res.update({'multi_auth': True,
                        'message': message,
                        })
        if len(auth_recs) == 1 and seq_number:
            res_number = auth_recs.agency + '-' + auth_recs.printer_point + '-' + self.fill_padding(seq_number,
                                                                                                    auth_recs.padding)
            res.update({
