...
class sri_xml_data(models.Model):
    ...
    number_document = fields.Char(u'Document Number', size=256, required=False, compute='_compute_document_number', store=True, index=True, help=u"",)
    file_xml_path = fields.Char(u'Ruta de Archivo XML', size=512, required=False, help=u"",)
    file_signed_path = fields.Char(u'Ruta de Archivo Firmado', size=512, required=False, help=u"",)
    file_authorized_path = fields.Char(u'Ruta de Archivo Autorizado', size=512, required=False, help=u"",)
    xml_file_version = fields.Char(u'Version XML', size=512, required=False, help=u"",)
    xml_key = fields.Char(u'Clave de Acceso', size=49, readonly=True, index=True, help=u"",)
    xml_authorization = fields.Char(u'Autorización SRI', size=49, readonly=True, index=True, help=u"",)
    description = fields.Char(u'Description', size=256, readonly=False, help=u"",)
    key_id = fields.Many2one('sri.keys', u'Clave Usada', required=False, index=True, auto_join=True, help=u"",)
    ...
    signed_date = fields.Datetime(u'Fecha de Firma', readonly=True, index=True, help=u"",)
    send_date = fields.Datetime(u'Fecha de Envío', readonly=True, help=u"",)
    response_date = fields.Datetime(u'Fecha de Respuesta', readonly=True, help=u"",)
    authorization_date = fields.Datetime(u'Fecha de Autorización', readonly=True, index=True, help=u"",)
    notification_active = fields.Boolean(string=u'Notificación de Documentos Electrónicos no Autorizados?', readonly=False, states={}, help=u"Esto permite activar o desactivar las notificaciones del presente documento", default = True) 
    ...
    type_emision = fields.Selection([('normal',u'Normal'),
                                    ...
                                    help=u"",)
    last_error_id = fields.Many2one('sri.error.code', u'Ultimo Mensaje de error', readonly=True, index=True, auto_join=True, help=u"",)
    send_message_ids = fields.One2many('sri.xml.data.message.line', 'xml_id', u'Mensajes Informativos', required=False, help=u"",)
    try_ids = fields.One2many('sri.xml.data.send.try', 'xml_id', u'Send Logs', required=False, help=u"",)
    ...
    external_document = fields.Boolean(u'Documento Externo?', readonly=True, help=u"",)
    process_now = fields.Boolean(u'Procesar Documento Externo?', default=True, readonly=True, help=u"",)
    fields_function_calculate = fields.Boolean(u'Campos funcionales calculados?', readonly=False, help=u"",)
    external_data = fields.Text(string=u'Informacion Externa importada', readonly=True, help=u"",)
    #campo para el numero de autorizacion cuando se cancelan documentos electronicos
    authorization_to_cancel = fields.Char(u'Autorización para cancelar', size=64, readonly=True, help=u"",)
    cancel_date = fields.Datetime(u'Fecha de cancelación', readonly=True, help=u"",)
    cancel_user_id = fields.Many2one('res.users', u'Usuario que canceló', readonly=True, help=u"",)

    _sql_constraints = [('invoice_out_id_uniq', 'unique (invoice_out_id)', _('Ya existe una factura electronica con el mismo numero!')),
                        ('credit_note_out_id_uniq', 'unique (credit_note_out_id)', _('Ya existe una Nota de credito electronica con el mismo numero!')),
                        ('debit_note_out_id_uniq', 'unique (debit_note_out_id)', _('Ya existe una Nota de debito electronica con el mismo numero!')),
                        ('withhold_id_uniq', 'unique (withhold_id)', _('Ya existe una Retencion electronica con el mismo numero!')),
                        ('delivery_note_id_uniq', 'unique (delivery_note_id)', _('Ya existe una Guia de remision electronica con el mismo numero!')),
                        ]
    ...
    def _get_company_id(self):
        ...
        if self.withhold_id and self.withhold_id.company_id: company_id = self.withhold_id.company_id.id 
        if self.delivery_note_id and self.delivery_note_id.company_id: company_id = self.delivery_note_id.company_id.id 
    ...
    def _get_ws_type_conection(self):
        ...
        if self.debit_note_out_id and self.debit_note_out_id.printer_id: printer_id = self.debit_note_out_id.printer_id.id 
        ...
        try:
            ...
            if conection_type == 'ws_receipt_production':
                ...
                if self._ws_receipt_production and self._ws_receipt_production.wsdl.url == ws_receipt_production:
                    ...
                    wsClient = Client(ws_receipt_production, timeout=company.ws_timeout)
            ...
            if conection_type == 'ws_auth_production':
                ...
                if self._ws_auth_production and self._ws_auth_production.wsdl.url == ws_auth_production:
                    ...
                    wsClient = Client(ws_auth_production, timeout=company.ws_timeout)
                    self._ws_auth_production = wsClient
        except Exception as e:
            error = self._clean_str(tools.ustr(e))
            _logger.warning(u"Error in Conection with web services of SRI, set in contingency mode. Error: %s", error)
        return wsClient
    ...
    def check_emision(self, environment):
        ...
        if not company.ws_receipt_test:
            raise Warning(_(u"Debe configurar la direccion del webservice de pruebas del SRI"))
        if not company.ws_receipt_production:
            raise Warning(_(u"Debe configurar la direccion del webservice de produccion del SRI"))
        ...
        if environment == '1':
            try:
                ...
                _logger.info(u"Conection Succesful with %s. Code %s", company.ws_receipt_test, code)
                if code == 200:
                    res = '1'
                else:
                    res = '2'
                ...
                error = self._clean_str(tools.ustr(e))
                _logger.warning(u"Error in Conection with %s, set in contingency mode. ERROR: %s", company.ws_receipt_test, error)
                #no pasar que es contingencia
                res = '1'
        elif environment == '2':
            try:
                code = urllib.request.urlopen(company.ws_receipt_production).getcode()
                _logger.info(u"Conection Succesful with %s. Code %s", company.ws_receipt_production, code)
                if code == 200:
                    res = '1'
                else:
                    res = '2'
                ...
                error = self._clean_str(tools.ustr(e))
                _logger.warning(u"Error in Conection with %s, set in contingency mode. ERROR: %s", company.ws_receipt_production, error)
                #no pasar que es contingencia
                res = '1'
        return res
    ...
    def generate_info_tributaria(self, xml_id, node, document_type, environment, emission, company, printer_id, sequence, date_document):
        ...
        if not clave_acceso:
            clave_acceso = key_model.get_single_key(key_id, document_type, environment, printer_id, sequence, emission, xml_data.xml_type, date_document) 
    ...
    def check_xsd(self, xml_string, xsd_file_name='factura_1.1.1.xsd'):
        try:
            xsd_file = tools.file_open(os.path.join('ecua_documentos_electronicos','data','xsd',xsd_file_name))
    ...
    def generate_xml_file(self, xml_id, document_id, invoice_type, type_grouped='individual'):
        ...
        send_mail_for_document = FIELD_FOR_SEND_MAIL_DOCUMENT[invoice_type] and getattr(company, FIELD_FOR_SEND_MAIL_DOCUMENT[invoice_type], False) or False
        ...
        root = Element(XML_HEADERS.get(invoice_type), id="comprobante", version=DOCUMENT_VERSIONS.get(invoice_type))
        ...
        if key_id:
            ...
            if environment == '1' and emission == '1':
                _logger.warning(u"Can't find key to generate xml file. No es posible encontrar claves de tipo pruebas para la generacion del archivo xml")
            elif environment == '2' and emission == '1':
                _logger.warning(u"Can't find key to generate xml file. No es posible encontrar claves de tipo producción para la generacion del archivo xml")
            #Si no tengo claves de contingencia no debo permitir seguir en el proceso, ya que despues no puedo generar el xml correctamente
                ...
                _logger.warning(u"Can't find key to generate xml file. No es posible encontrar claves de tipo contingencia para la generacion del archivo xml, no se puede continuar con la operación actual")
    ...
    def _create_messaje_response(self, xml_id, messajes, authorized, raise_error):
        ...
        for message in messajes:
            ...
            if not authorized and message.get('identificador') and message.get('identificador') in ('70',70):
                _logger.warning(u"Clave 70, en espera de autorizacion. %s %s", message.get('mensaje',''), message.get('informacionAdicional',''))
    ...
    def _send_xml_data_to_valid(self, xml_id, xml_field, client_ws, client_ws_auth):
        ...
        try:
            ...
            if len(xml_rec.try_ids) >= 1:
                ...
                ok, msgs = self._process_response_autorization(xml_id, responseAuth)
                if ok:
                    response = {
                               'estado': 'RECIBIDA',
                               }
            ...
            if send:
                ...
                xml_string = base64.encodestring(self.get_file(xml_id, xml_field)).decode()
                response = client_ws.service.validarComprobante(xml = xml_string)
                try_model.write({'response_date': time.strftime(DTF)})
                _logger.info(u"Send file succesful, claveAcceso %s. %s", xml_rec.xml_key, str(response.estado) if hasattr(response,'estado') else u'SIN RESPUESTA')
            ...
            error = self._clean_str(tools.ustr(e))
            _logger.info(u"can\'t validate document in %s, claveAcceso %s. ERROR: %s", str(client_ws), xml_rec.xml_key, error)
            tr = self._clean_str(tools.ustr(traceback.format_exc()))
            _logger.info(u"can\'t validate document in %s, claveAcceso %s. TRACEBACK: %s", str(client_ws), xml_rec.xml_key, tr)
            response = False
            xml_rec.write({'state': 'waiting'})
            ok = False
        return response
    ...
    def _process_response_check(self, xml_id, response):
        ...
        if response and not isinstance(response, dict):
            ...
            try:
                ...
                error = self._clean_str(tools.ustr(e))
                _logger.info(u"can\'t validate document, claveAcceso %s. ERROR: %s", xml_rec.xml_key, error)
                tr = self._clean_str(tools.ustr(traceback.format_exc()))
                _logger.info(u"can\'t validate document, claveAcceso %s. TRACEBACK: %s", xml_rec.xml_key, tr)
                ok = False
    ...
    def _send_xml_data_to_autorice(self, xml_id, xml_field, client_ws):
        ...
        xml_rec = self.browse(xml_id)
        try:
            response = client_ws.service.autorizacionComprobante(claveAccesoComprobante = xml_rec.xml_key)
        except WebFault as ex:
            response = False
            xml_rec.write({'state': 'waiting'})
            error = self._clean_str(tools.ustr(ex))
            ...
            error = self._clean_str(tools.ustr(e))
            _logger.warning(u"Error send xml to server %s. ERROR: %s", client_ws, error)
        return response
    ...
    def _process_response_autorization(self, xml_id, response):
        ...
        if not response:
            ...
            _logger.warning(u"Authorization response error, No response get. Documento en espera de autorizacion")
            xml_rec.write({'state': 'waiting'})
            return ok, msj_res
        if isinstance(response, object) and not hasattr(response, 'autorizaciones'):
            #si no tengo respuesta, dejar el documento en espera de autorizacion, para que la tarea cron se encargue de procesarlo y no quede firmado el documento
            _logger.warning(u"Authorization response error, No Autorizacion in response. Documento en espera de autorizacion")
            xml_rec.write({'state': 'waiting'})
            return ok, msj_res
        ...
        if isinstance(response.autorizaciones, (str)):
            _logger.warning(u"Authorization data error, reponse message is not correct. %s", str(response.autorizaciones))
            dump(response)            
            return ok, msj_res
        ...
        for doc in autorizacion_list:
            ...
            if estado == 'AUTORIZADO' and not no_write:
                ...
                _logger.info(u"Authorization succesful, claveAcceso %s. Autohrization: %s", xml_rec.xml_key, str(numeroAutorizacion))
            ...
            try:
                ...
                error = self._clean_str(tools.ustr(e))
                _logger.warning(u"Can\'t process messages %s. ERROR: %s", doc.mensajes, error)
        return ok, msj_res
    ...
    def create_file_authorized(self):
        for xml_data in self:
            ...
            if  xml_data.state == 'authorized' and xml_data.xml_authorization and xml_data.file_signed_path:
                tree = etree.parse(self.generate_file_name(xml_data.id, 'file_signed'))
                ...
                authorizacion_ele.text = xml_data.authorization_date or time.strftime(DTF)
                root.append(authorizacion_ele)
    ...
    def test_send_check_state(self):
        ...
        max_intentos=1
        for xml_rec in self:
            xml_rec.with_context(no_send=True).send_xml_data_to_check(xml_rec.id, environment, xml_field, max_intentos)
        return True
    ...
    def send_xml_data_to_check(self, xml_id, environment, xml_field='file_signed', max_intentos=1):
        ...
        if ws_receipt:
            try:
                receipt_client = self.get_current_wsClient('ws_receipt_' + type_url)
                ...
                if not res_ws_valid and not raise_error:
                    ...
                    if not response_auth or isinstance(response_auth.autorizaciones, (str)) :
                        ...
                        res_ws_valid, msj, raise_error, previous_authorized = self._process_response_check(xml_id, response_check)
                        ...
                        authorized, msj = self._process_response_autorization(xml_id, response_auth)
                        message_data.extend(msj)
                messages_error, raise_error = self._create_messaje_response(xml_id, message_data, authorized, raise_error)
            except Exception as e:
                xml_rec.write({'state': 'rejected'})
                #FIX: pasar a unicode para evitar problemas 
                error = self._clean_str(tools.ustr(e))
                _logger.warning(u"Error send xml to server %s. ERROR: %s", ws_receipt, error)
                send_again = True
        if send_again:
        ...
        if messages_error and raise_error and environment == '2':
            ...
            messages_error.insert(0, u"No se pudo autorizar, se detalla errores recibidos")
            raise Warning(_(u"\n".join(messages_error)))
        return authorized
    ...
    def __get_variables_availables(self, document, invoice_type):
        ...
        res['TipoDocumento'] = util_model.get_selection_item('sri.mail.message', 'document_type', invoice_type)
...
#         ctx['no_objects'] = True
        ...
        document_type = modules_mapping.get_document_type(invoice_type)
        model_name = modules_mapping.get_model_name(document_type)
        message_recs = message_model.search([('document_type','=',invoice_type)])
        if message_recs and model_name:
            document = self.env[model_name].browse(document_id)
            valiables_availables = self.__get_variables_availables(document, invoice_type)
            #TODO: agregar url
            ...
            if document.partner_id:
                res['email_to'] = partner_mail_model.get_email_to_partner(document.partner_id.id, invoice_type)
            #generar el reporte
            ...
            res['report_file'] = util_model.with_context(ctx).create_report([document_id], report_name, model_name, file_name_report)[0]
        return res
    ...
    def get_info_xml_to_partner(self, document_id, invoice_type='out_refund'):
        ...
        if sri_xml_recs:
            ...
            #TODO: obtener el xml
            file_name_report = REPORT_NAME[invoice_type] + document.document_number
            xml_file = self.get_file(sri_xml.id, 'file_authorized') 
            if xml_file:
                res['xml_file'] = (file_name_report + ".xml", base64.encodestring(xml_file))
        return res
    ...
    def _check_totales(self, xml_data, document, document_type):
        ...
        notification_id = mod_model.xmlid_to_res_id('ecua_documentos_electronicos.not_elect_002', False)
        if notification_id:
            try:
            ...
            if external_data and document_type in ('withhold','out_invoice'):
                ...
                if document_type == 'out_invoice':
                    ...
                    total_importado = float(external_data.get('importe_total', 0.0))
    ...
    def text_write_binary(self):
        ...
        for xml_rec in self:
            ...
            if res_document:
                invoice_type = modules_mapping.get_invoice_type(res_document.type, res_document.debit_note, res_document.liquidation)
            elif xml_rec.withhold_id:
            ...
            if res_document and invoice_type:
                try:
                    if xml_rec.external_document and self.env.context.get('call_from_cron', False):
                        task_recs = self._check_totales(xml_rec, res_document, invoice_type)
                        if task_recs:
                            notification_task_recs += task_recs
                            continue
                    new_ids.append(xml_rec.id)
                    string_data, binary_data = self.generate_xml_file(xml_rec.id, res_document.id, invoice_type, 'individual')
                    ...
                    error = self._clean_str(tools.ustr(e))
                    _logger.warning(u"Error function text_write_binary. ERROR: %s", error)
    ...
    def send_mail_document_wrong_total(self, notification_task_recs):
        ...
        if notification_task_recs:
            ...
            messages = notification_task_model.get_messages(notification_task_recs.ids, True).values()
            for message_mail in messages: 
                util_model.send_notification_email('not_elect_002', 'ecua_documentos_electronicos', content=message_mail)
    ...
    def test_sing_xml_file(self):
        def _print_error(er):
            if xml_rec.external_document:
                ...
                _logger.warning(u"Error sing xml data ID: %s. ERROR: %s", xml_rec.id, error)
            else:
                raise
        ...
        for xml_rec in self:
            ...
            try:
                if not company.key_type_id:
                    if company.type_environment == 'production':
                        raise Warning(_(u"Es obligatorio seleccionar el tipo de llave o archivo de cifrado usa para la firma de los documentos electrónicos, verificar la configuración de la compañia"))
                    else:
                        _logger.warning(u"Error key type doesn't configured correctly in company, please verify")
                        return False
                ...
                if company.key_type_id.key_type == 'file':
                    ...
                    if company.key_type_id.path:
                        ...
                        cert_path = os.path.join('ecua_documentos_electronicos','data','xml_samples',cert_file_name)
                ...
                if xml_rec.file_xml_path:
                    ...
                    xml_signed = ws_signer.service.signXmlFile(xml_string_data.decode(), cert_path, cert_pass, cert_type)
                    file_signed_path = self.write_file(xml_rec.id, 'file_signed', str.encode(xml_signed))
                    vals = {
        ...
        for xml_rec in self:
            ...
            try:
                ...
                if compose_data.get('res_model', False):
                    wizard_model = self.env[compose_data.get('res_model', False)]
                    wizard = wizard_model.with_context(compose_data.get('context')).create({})
    ...
    def _create_invoice_tax(self, invoice_id, values, company, company_currency):
        for taxe in values:
            ...
            taxe['base'] = company_currency.round(trunc_decimal(taxe['base'], 3))
            taxe['amount'] = company_currency.round(trunc_decimal(taxe['amount'], 3))
            taxe['base_amount'] = company_currency.round(trunc_decimal(taxe['base_amount'], 3))
            taxe['tax_amount'] = company_currency.round(trunc_decimal(taxe['tax_amount'], 3))
        ...
        for xml_data in xml_recs:
            ...
            if not ok and messages:
                ...
                for msj in messages:
                    ...
                    message_mail.append("<td>%s</td>" % msj.get('informacionAdicional',''))
                    message_mail.append("</tr>")
                message_mail.append("</table><br>")
        if send_mail:
            util_model.send_notification_email('not_elect_001','ecua_documentos_electronicos', "".join(message_mail),'', '',[], '')
        return True
...
#         args.append(('state','in',('authorized',)))
        ...
        if args:
            ...
            if not xml_recs:
                res['mensajes'].append('No se encontro archivo con los datos facilitados')
            else:
                res['file'] = base64.encodestring(self.get_file(xml_recs[0].id, field_name))
        return res
        ...
        if args:
            ...
            if not xml_recs:
                ...
                if res_id and model and report_name:
                    ...
                    file_report = util_model.with_context(ctx).create_report([res_id], report_name, model, file_name)
                    if file_report:
    ...
    def send_mail_to_partner(self):
        ...
        if xml_ids:
            documents_sended, documents_no_sended = self.with_context(ctx).browse(xml_ids).test_send_mail_partner()
            if documents_sended:
            ...
            if documents_no_sended:
                ...
                self.env.cr.execute(SQL, {'xml_ids': tuple(documents_no_sended)})
        return True
    ...
    def generate_file_name(self, xml_id, file_type):
        ...
        if file_type not in ('file_xml','file_signed','file_authorized'):
            raise Warning(_(u"Tipo de archivo no valido, se permite signed, authorized. Por favor verifique"))
        xml_rec = self.browse()
        ...
        if field_name in self._fields and xml_rec[field_name] and not self.env.context.get('only_file', False):
            ...
            file_name = "%s_%s_%s_%s.xml" % (xml_id, document_type, document_number, file_type)
        return file_name
    ...
    def get_file(self, xml_id, file_type):
        ...
        if not root_path:
            raise Warning(_(u"Debe configurar la ruta para guardar los documentos electronicos. Por favor verifique en la configuracion de compañia"))
        file_data = ""
        ...
        if os.path.isfile(full_path):
            try:
                ...
                _logger.warning(_(u"No se puede leer el archivo %s, verifique permisos en la ruta %s!" % (file_name, root_path)))
        else:
            raise Warning(_(u"Archivo %s no encontrado en la ruta %s") % (file_name, root_path))
        return file_data
    ...
    def write_file(self, xml_id, file_type, file_content):
        ...
        try:
            ...
            _logger.warning(_(u"No se puede escribir en el archivo %s, verifique permisos en la ruta %s!" % (file_name, root_path)))        
        return full_path
    ...
    def get_file_to_wizard(self):
        ...
        if not file_data:
            ...
            res = util_model.with_context(ctx).show_wizard(wizard_model._name, 'wizard_xml_get_file_form_view', _(u'Descargar Archivo'))
        ...
        for xml_data in self:
            ...
            if xml_data.state != 'draft':
                ...
                if xml_data.state == 'cancel' and not xml_data.authorization_to_cancel:
                    continue
                raise Warning(_(u"No puede eliminar registros a menos que esten en estado borrador"))
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False, access_rights_uid=None):
        ...
        for domain in args:
            if len(domain) == 3:
                ...
                new_domain.append(domain)   
        res = super(sri_xml_data, self)._search(new_domain, offset=offset, limit=limit, order=order, count=count, access_rights_uid=access_rights_uid)
        return res


    def get_electronic_logo_image(self):
        self.ensure_one()
        if self.shop_id.electronic_logo: return self.shop_id.electronic_logo
        if self.company_id.electronic_logo: return self.company_id.electronic_logo
        if self.company_id.logo: return self.company_id.logo
        return False
...
class sri_xml_data_message_line(models.Model):
    ...
    xml_id = fields.Many2one('sri.xml.data', u'XML Data', required=True, index=True, help=u"",)
    message_code_id = fields.Many2one('sri.error.code', u'Código de Mensaje', required=True, index=True, help=u"",)
    message_type = fields.Char(u'Tipo', size=64, required=False, help=u"",)
    other_info = fields.Text(string=u'Información Adicional', required=False, help=u"",)
    message = fields.Text(string=u'Mensaje', required=False, help=u"",)
    create_date = fields.Datetime(u'Fecha de Creación', readonly=True, help=u"",)
    write_date = fields.Datetime(u'Ultima actualización', readonly=True, help=u"",)
...
class sri_xml_data_send_try(models.Model):
    ...
    xml_id = fields.Many2one('sri.xml.data', u'XML Data', required=False, index=True, help=u"",)
